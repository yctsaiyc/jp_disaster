...
class ETL_VPCK70(ETL_jp_disaster):
    def xml_to_df(self, xml_path, soup):
        ...
        # “区域予報”の場合は、地域平均気温(2内容部の個別要素の詳細※1参照)、“地点予報”の場合は、
        # 最高気温・最低気温に関する情報(2内容部の個別要素の詳細※2参照)を記述する。
        #
        # └ TimeSeriesInfo
        ...
        # └ jmx_eb:Comparison 前後2日5日間平均・地域平均気温の平年偏差階級を-3から3の階級を記述する。属性
        # refID は、予報対象日の参照番号を記述するもので、TimeDefines で定義した timeId に
        # 対応する。属性 description には階級値の文字列表現が入る。
        #
        # └ Area 予報対象地域を記述する。※
        ...
        #             └ jmx_eb:Temperature 予想気温の期間平均値を記述する。Type 要素の値が“最低気温”、“最低気温予測範囲”、“最高気温”、“最
        #                                 高気温予測範囲”の場合に出現する。属性 type の値により、予想気温の内容を期間平均値で示す。属性 unit
        #                                 は気温の単位を示す。属性 refID は、予報対象日の参照番号を記述する。TimeDefines で定義した timeId
        #                                 に対応する。属性 description には予報値の文字列表現が入る。
        #
        # └ jmx_eb:Comparison 気温の期間平均値の予想階級を記述する。Type 要素が“最低気温階級”又は“最高気温階級”の場合に出
        # 現する。期間平均した最高・最低気温の平年偏差階級を-3から3の階級を記述する。属性 refID は、予報
        # 対象日の参照番号を記述するもので、TimeDefines で定義した timeId に対応する。属性 description には
        # 階級値の文字列表現が入る。
        ...
        # で示す。
        # 左の例では、
        # 発表日の 8 日先(refID="1")の最高
        # 気温の 5 日間平均値が 24 度で、23 度
        # ~27 度の範囲に予想されていること
        # を示す。
        return df
